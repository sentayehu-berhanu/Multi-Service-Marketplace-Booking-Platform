...
const DashboardAutoServices = () => {
  ...
  const fetchBusinessAndServices = async () => {
    try {
      ...
      if (bizRes.data && bizRes.data.length > 0) {
        ...
        // Ensure this page is only used by Auto businesses
        if (business.category?.slug !== 'auto' && business.category?.slug !== 'car-wash') {
      ...
      setError("Failed to load auto services.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const openAddModal = () => {
    ...
    setFormData({ name: '', serviceType: 'Car Wash', description: '', price: '', duration: '60', status: 'ACTIVE' });
    setIsModalOpen(true);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error('Error saving auto service:', err);
      alert('Failed to save service.');
    }
  };

  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this auto service?")) {
      try {
        const token = localStorage.getItem('token');
        ...
        console.error('Error deleting auto service:', err);
        alert('Failed to delete service.');
      }
    }
  };

  if (loading) return <div style={{ textAlign: 'center', padding: '3rem' }}>Loading auto services...</div>;
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#0f172a', padding: '2rem', borderRadius: '15px', color: 'white' }}>
        <div>
          <h1 style={{ fontSize: '2rem', margin: '0 0 10px 0' }}>Auto Service Builder</h1>
          <p style={{ margin: 0, color: '#94a3b8' }}>Manage your car washes, oil changes, detailing, and repairs.</p>
        </div>
        <button 
      ...
      <div className="glass-panel" style={{ padding: '2rem' }}>
        {services.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '3rem', color: 'var(--text-secondary)' }}>
            No auto services found. Add your first service to get started!
          </div>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', maxWidth: '800px' }}>
            {services.map(service => (
              <div key={service.id} className="glass-panel hover-scale" style={{ padding: '1.5rem 2rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderRadius: '15px', borderLeft: '4px solid #2563eb' }}>
                <div style={{ flex: 1 }}>
      ...
      {isModalOpen && (
        <div style={{ 
          ...
          <div style={{ background: 'white', width: '100%', maxWidth: '500px', padding: '2.5rem', borderRadius: '15px', position: 'relative' }}>
            <h2 style={{ marginBottom: '1.5rem', marginTop: 0, color: '#0f172a' }}>{editingService ? 'Edit Service' : 'Add Auto Service'}</h2>
            
            <form onSubmit={handleSubmit}>
              <div style={{ marginBottom: '15px' }}>
                <label style={{ display: 'block', marginBottom: '5px', color: '#475569', fontWeight: 'bold', fontSize: '0.9rem' }}>Service Name (e.g. Premium Wash)</label>
...
export default DashboardAutoServices;
